...
describe("core", function () {
  ...
  it("2) add collection", async function () {
    ...
    expect((await marketPlaceCollection.addressToCollectionId(owner.address)).toString()).to.equal("1");
  })
  ...
  it("4) buy with contract", async function () {
    ...
    expect((await nfticketHelper2.balanceOf(owner.address)).toString()).to.equal("0");
    ...
    expect((await nfticketHelper2.balanceOf(owner.address)).toString()).to.equal("1");
  ...
  it("5) buy through stake market", async function () {
    ...
    expect((await nfticketHelper2.balanceOf(owner.address)).toString()).to.equal("1");
    expect((await ust.balanceOf(stakeMarket.address)).toString()).to.equal('0');
    // create stake
    ...
    expect((await ust.balanceOf(stakeMarket.address)).toString()).to.equal("10");
    expect((await stakeMarket.stakesBalances(1)).toString()).to.equal("10");
    // apply to stake
    ...
    expect((await stakeMarket.stakesBalances(1)).toString()).to.equal("10");
    expect((await stakeMarket.getStake(1)).parentStakeId.toString()).to.equal("1");
    expect((await stakeMarket.getStake(2)).parentStakeId.toString()).to.equal("1");
    expect((await stakeMarketNote.getDuePayable(2,0))[0].toString()).to.equal("10");
    // merchant processes transaction
    await stakeMarket.unlockStake(2, 0, false);
    expect((await nfticketHelper2.balanceOf(owner.address)).toString()).to.equal("2");
  ...
  it("6) create lock", async function () {
    ...
    expect((await ve.balanceOf(owner.address)).toString()).to.equal("0");
    await ve.create_lock(ve_underlying_amount, lockDuration);
    expect(await ve.ownerOf(1)).to.equal(owner.address);
    expect((await ve.balanceOf(owner.address)).toString()).to.equal("1");
  ...
  it("7) resolve conflict through stake market", async function () {
    ...
    expect((await ust.balanceOf(stakeMarket.address)).toString()).to.equal("91") //gasPercent=10% goes to voters
    // vote
    ...
    expect((await stakeMarket.getStake(3)).ownerAgreement.toString()).to.equal("4")
    expect((await stakeMarket.getStake(4)).ownerAgreement.toString()).to.equal("4")
    expect((await stakeMarket.getStake(3)).bank.amountReceivable.toString()).to.equal("0")
    expect((await stakeMarket.getStake(3)).bank.amountPayable.toString()).to.equal("100")
    expect((await stakeMarket.getStake(4)).bank.amountReceivable.toString()).to.equal("0")
    expect((await stakeMarket.getStake(4)).bank.amountPayable.toString()).to.equal("100")
    // update 
    ...
    expect((await stakeMarket.stakeStatus(3)).winnerId.toString()).to.equal("4")
    expect((await stakeMarket.stakeStatus(3)).loserId.toString()).to.equal("3")
    ...
    await expectRevert.unspecified(
      stakeMarket.updateStatusOrAppeal(4, "Appeal", "Appealing decision", "")
    );// winner cannot appeal
    ...
    expect((await stakeMarket.stakeStatus(3)).winnerId.toString()).to.equal("3")
    expect((await stakeMarket.stakeStatus(3)).loserId.toString()).to.equal("4")
    ...
    expect((await stakeMarketVoter.litigationId()).toString()).to.equal("3") 
    await stakeMarket.updateStatusOrAppeal(3, "", "", "")
    expect((await stakeMarket.stakeStatus(3)).status).to.equal(0) //peace
    expect((await stakeMarketVoter.litigationId()).toString()).to.equal("3") // same litigation id
    ...
    expect((await stakeMarket.getStake(3)).ownerAgreement.toString()).to.equal("2")
    expect((await stakeMarket.getStake(4)).ownerAgreement.toString()).to.equal("2")
    expect((await stakeMarket.getStake(3)).bank.amountReceivable.toString()).to.equal("0")
    expect((await stakeMarket.getStake(3)).bank.amountPayable.toString()).to.equal("100")
    expect((await stakeMarket.getStake(4)).bank.amountReceivable.toString()).to.equal("100")
    expect((await stakeMarket.getStake(4)).bank.amountPayable.toString()).to.equal("0") 
    ...
    expect((await ust.balanceOf(owner.address)).toString()).to.equal("999999999999999962") // 10% of the 100 went to voters when creating gauge
    expect((await ust.balanceOf(stakeMarket.address)).toString()).to.equal("0")
    expect((await stakeMarketNote.getDuePayable(3,0))[0].toString()).to.equal("9")
  ...
  it("8) create and set up valuepool", async function () {
    ...
    expect((await va.balanceOfNFT(1)).toString()).to.equal("0")
    await va.create_lock_for(ethers.BigNumber.from("100000000000000000"), 4 * 365 * 86400, 0, owner.address)
    expect((await va.balanceOfNFT(1)).toString()).to.not.equal("0")
  ...
  it("9) create bounty & add balance", async function () {
    ...
    expect((await trustBounties.getBalance(1)).toString()).to.equal("10");
  ...
  it("10) Create and Add sponsor", async function () {
    ...
    expect((await sponsor.protocolInfo(1)).amountPayable.toString()).to.equal("1000000000000000000")
    ...
    expect((await sponsorNote.getDuePayable(sponsor.address, vava.address, 0))[0].toString()).to.equal("1000000000000000000")
    await vava.notifyPayment(sponsor.address)
    console.log("due============>", await sponsorNote.getDuePayable(sponsor.address, vava.address, 0))
    expect((await sponsorNote.getDuePayable(sponsor.address, vava.address, 0))[0].toString()).to.equal("0")
  ...
  it("11) buy item with valuepool", async function () {
    ...
    expect((await vava.getQueue(rank)).length.toString()).to.equal("1");
    expect((await nfticketHelper2.balanceOf(owner.address)).toString()).to.equal("2");
    await vava.executeNextPurchase()
    expect((await vava.getQueue(rank)).length.toString()).to.equal("0");
    expect((await nfticketHelper2.balanceOf(owner.address)).toString()).to.equal("3");
  ...
  it("13) buy with identityproof", async function () {
    ...
    expect((await nfticketHelper2.balanceOf(owner.address)).toString()).to.equal("3");
    await expectRevert.unspecified(
      marketPlaceTrades.buyWithContract(
        owner.address,
        owner.address,
        "0x0000000000000000000000000000000000000000",
        "uber",
    ...
    expect((await nfticketHelper2.balanceOf(owner.address)).toString()).to.equal("4");
  ...
  it("14) add unique accounts", async function () {
    ...
    expect((await profile.addressToProfileId(owner3.address)).toString()).to.equal("1");

    // fails because the ssid attached to token id 5 has already been used
    await expectRevert.unspecified(
      marketPlaceTrades.connect(owner2).buyWithContract(
      owner.address,
      owner2.address,
      "0x0000000000000000000000000000000000000000",
      "uber",
  ...
  it("15) Modify Ask order", async function () {
    ...
    expect((await marketPlaceOrders.getAskDetails(1,'0x7380266989d9485aa283717a6eb249d7d56d141257e11deabb1a892587361f96')).price.toString()).to.equal("11")
    expect((await marketPlaceOrders.getAskDetails(1,'0x7380266989d9485aa283717a6eb249d7d56d141257e11deabb1a892587361f96')).maxSupply.toString()).to.equal("11")
  ...
  it("16) Buy with options", async function () {
    ...
    await expect((await marketPlaceHelper.getRealPrice(owner.address,owner.address,"uber",[],0,11))[0].toString()).to.equal("11")
    await expect((await marketPlaceHelper.getRealPrice(owner.address,owner.address,"uber",[],0,11))[1]).to.equal(false)
    await expect((await marketPlaceHelper.getRealPrice(owner.address,owner.address,"uber",[0],0,11))[0].toString()).to.equal("12")
    await expect((await marketPlaceHelper.getRealPrice(owner.address,owner.address,"uber",[0],0,11))[1]).to.equal(false)
    await expect((await marketPlaceHelper.getRealPrice(owner.address,owner.address,"uber",[1],0,11))[0].toString()).to.equal("13")
    await expect((await marketPlaceHelper.getRealPrice(owner.address,owner.address,"uber",[1],0,11))[1]).to.equal(false)
    await expect((await marketPlaceHelper.getRealPrice(owner.address,owner.address,"uber",[0,1],0,11))[0].toString()).to.equal("14")
  ...
  it("17) Buy discounted with account limits", async function () {
    ...
    expect((await marketPlaceOrders.getAskDetails(1,'0x7380266989d9485aa283717a6eb249d7d56d141257e11deabb1a892587361f96')).priceReductor.discountStatus.toString()).to.equal("1")
    expect((await marketPlaceHelper.getRealPrice(owner.address,owner.address,"uber",[],0,11))[0].toString()).to.equal("9") //2% discount
    expect((await marketPlaceHelper.getRealPrice(owner.address,owner.address,"uber",[],0,11))[1]).to.equal(true)
    expect((await marketPlaceHelper.getRealPrice(owner.address,owner.address,"uber",[0],0,11))[0].toString()).to.equal("10") //2% discount
    expect((await marketPlaceHelper.getRealPrice(owner.address,owner.address,"uber",[0],0,11))[1]).to.equal(true)
    expect((await marketPlaceHelper.getRealPrice(owner.address,owner.address,"uber",[1],0,11))[0].toString()).to.equal("11") //2% discount
    expect((await marketPlaceHelper.getRealPrice(owner.address,owner.address,"uber",[1],0,11))[1]).to.equal(true)
    expect((await marketPlaceHelper.getRealPrice(owner.address,owner.address,"uber",[0,1],0,11))[0].toString()).to.equal("12") //2% discount
    ...
    expect((await nfticketHelper2.balanceOf(owner.address)).toString()).to.equal("5");
    ...
    expect((await marketPlaceHelper.getRealPrice(owner.address,owner.address,"uber",[],0,11))[0].toString()).to.equal("11") //discount no longer available
  ...
  it("18) Reinitialize discounts w/o identity", async function () {
    expect((await marketPlaceHelper.getRealPrice(owner.address,owner.address,"uber",[],0,11))[0].toString()).to.equal("11") //2% discount no more available
    ...
    expect((await marketPlaceHelper.getRealPrice(owner.address,owner.address,"uber",[],0,11))[0].toString()).to.equal("9") //2% discount available again
    expect((await marketPlaceHelper.getRealPrice(owner.address,owner.address,"uber",[],0,11))[1]).to.equal(true)
  ...
  it("19) Buy discounted with identity limits", async function () {
    ...
    expect((await marketPlaceHelper.getRealPrice(owner.address,owner.address,"uber",[],3,11))[0].toString()).to.equal("9") //2% discount
    ...
    expect((await nfticketHelper2.balanceOf(owner.address)).toString()).to.equal("6");
    expect((await marketPlaceHelper.getRealPrice(owner.address,owner.address,"uber",[],3,11))[0].toString()).to.equal("11") //discount no longer available
  ...
  it("20) Receive cashback with account limits", async function () {
    ...
    expect((await marketPlaceOrders.getPaymentCredits(owner.address,1,"uber")).toString()).to.equal("4")
    await expectRevert.unspecified(
      marketPlaceTrades.processCashBack(
      owner.address, 
      "uber",
      true,
      "uber"
  ...
  it("21) Reinitialize discounts w/ identity", async function () {
    ...
    expect((await marketPlaceHelper.getRealPrice(owner.address,owner.address,"uber",[],3,11))[0].toString()).to.equal("9") //2% discount available again
    expect((await marketPlaceHelper.getRealPrice(owner.address,owner.address,"uber",[],3,11))[1]).to.equal(true)
  })
  ...
  it("23) Receive cashback in cash", async function () {
    ...
    expect((await ust.balanceOf(owner.address)).toString()).to.equal("899999999999999915");
  ...
  it("24) Register user with identity limits", async function () {
    ...
    //     await expectRevert.unspecified(marketPlaceEvents.emitUserRegistration(1, 1, 5, 1, true))
    //     await expectRevert.unspecified(marketPlaceEvents.emitUserRegistration(1, 1, 5, 0, true))
  ...
  it("27) create recurring bounty & add balance", async function () {
    ...
    expect((await trustBounties.getBalance(2)).toString()).to.equal("10");
  })
  ...
  it("29) create gauge", async function () {
    ...
    expect((await businessVoter.poolForGauge(gauge)).toString()).to.equal("1")
    ...
    expect((await businessVoter.pools(ve.address,0)).toString()).to.equal("1")
  ...
  it("30) Effects of Buy - vote/nfticket/lottery/treasury/cashbackfund/recurring revenue ", async function () {
    ...
    expect((await nfticketHelper2.balanceOf(owner2.address)).toString()).to.equal("1");
    expect((await marketPlaceTrades.recurringBountyBalance(1,ust.address)).toString()).to.equal("0");
    expect((await marketPlaceOrders.getPaymentCredits(owner.address,1,"uber")).toString()).to.equal("8")
    expect((await marketPlaceHelper.getRealPrice(owner.address,owner.address,"uber",[],3,100))[0].toString()).to.equal("80")
    expect((await marketPlaceTrades.pendingRevenue(ust.address,1)).toString()).to.equal("65")
    expect((await marketPlaceTrades.lotteryRevenue(ust.address)).toString()).to.equal("0")
    expect((await marketPlaceTrades.cashbackFund(ust.address,1)).toString()).to.equal("0")
    expect(await businessBribe.ownerOf(2)).to.equal("0x0000000000000000000000000000000000000000")
    expect((await businessBribe.balanceOf(2)).toString()).to.equal("0")
    expect((await businessVoter.weights(1,ve.address)).toString()).to.equal("0")
    ...
    expect(gauge2).to.not.equal("0x0000000000000000000000000000000000000000")
    expect(await referralVoter.bribes(gauge2)).to.not.equal("0x0000000000000000000000000000000000000000")
    expect((await referralVoter.poolForGauge(gauge2)).toString()).to.equal("1")
    expect(await referralVoter.isGauge(gauge2)).to.equal(true)
    expect((await referralVoter.pools(ve.address,0)).toString()).to.equal("1")
    expect((await referralVoter.weights(2,ve.address)).toString()).to.equal("0")
    expect((await referralVoter.pools(ve.address,0)).toString()).to.equal("1")
    expect((await nfticketHelper.lotteryCredits(owner.address, ust.address)).toString()).to.equal("0")
    ...
    // expect((await nfticketHelper.lotteryCredits(owner2.address, ust.address)).toString()).to.equal("8")
    // expect((await referralVoter.weights(1,ve.address)).toString()).to.not.equal("0")
    expect((await referralBribe.balanceOf(1)).toString()).to.not.equal("0")
    expect((await businessBribe.balanceOf(1)).toString()).to.not.equal("0")
    expect(await businessBribe.ownerOf(1)).to.equal(refAddress)
    expect((await businessVoter.weights(1,ve.address)).toString()).to.not.equal("0")
    expect((await marketPlaceOrders.getPaymentCredits(owner2.address,1,"uber")).toString()).to.equal("0")
    expect((await marketPlaceTrades.recurringBountyBalance(1,ust.address)).toString()).to.equal("8");
    expect((await marketPlaceTrades.pendingRevenue(ust.address,1)).toString()).to.equal("112") //65 + 8 + 39 (netPrice + referrerFee)
    expect((await marketPlaceHelper.getRealPrice(owner.address,owner2.address,"uber",[],3,100))[0].toString()).to.equal(100)
    expect((await marketPlaceTrades.lotteryRevenue(ust.address)).toString()).to.equal("8")
    expect((await marketPlaceTrades.cashbackFund(ust.address,1)).toString()).to.equal("9")
    expect((await nfticketHelper2.balanceOf(owner2.address)).toString()).to.equal("2");
    // reset
  ...
  it("31) Mint external nfticket", async function () {
    ...
    expect((await nfticketHelper2.balanceOf(owner.address)).toString()).to.equal("7");
    expect((await nfticketHelper2.balanceOf(owner2.address)).toString()).to.equal("2");
  ...
  it("32) Burn FT for credit", async function () {
    ...
    expect(marketPlaceOrdersBalanceBefore.toString()).to.equal("0")
    ...
    console.log("payment credits after============>", await marketPlaceOrders.getPaymentCredits(owner.address,1,"uber"))
    expect((await marketPlaceOrders.getPaymentCredits(owner.address,1,"uber")).toString()).to.equal("18")
    expect((await nfticketHelper2.balanceOf(owner2.address)).toString()).to.equal("2");
  ...
  // await expectRevert.unspecified(
  //     marketPlaceHelper.connect(owner2).processAuction(
  //     owner.address,
  //     owner.address,
  //     owner2.address,
  //     "Chimpanzee",
