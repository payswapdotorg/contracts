...
contract MockRandomNumberGenerator {
     ...
     */
    function randomnessIsRequestedHere(uint32 round, address user) external {
     ...
     */
    function changeLatestLotteryId(uint _id) external {
        latestLotteryId = _id;
    }
    ...
    function viewRandomNumbers(
        uint _round, 
        address _user
    ) external view returns (uint[] memory result) {
        result = new uint[](1);
        result[0] = randomResult; 
        return result;
    }
