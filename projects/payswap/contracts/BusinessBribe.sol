...
contract BusinessBribe {
    ...
    */
    ...
    function getReward(uint tokenId, address[] memory tokens) external lock  {
        require(IProfile(IContract(contractAddress).profile()).addressToProfileId(msg.sender) == tokenId, "Not profile owner");
    ...
    function getRewardForOwner(uint tokenId, address[] memory tokens) external lock  {
        require(msg.sender == IContract(contractAddress).businessVoter(), "Only business voter");
    ...
    function deposit(uint amount, uint tokenId, address _owner) external {
        require(msg.sender == IContract(contractAddress).businessVoter(), "Only business voter");
        totalSupply += amount;
    ...
    function notifyRewardAmount(address token, uint amount) external lock {
        ...
        if (block.timestamp >= periodFinish[token]) {
            ...
            _adminFee = amount * IContract(contractAddress).adminShare() / 10000;
            amount -= _adminFee;
            rewardRate[token] = amount / DURATION;
        } else {
            uint _remaining = periodFinish[token] - block.timestamp;
            uint _left = _remaining * rewardRate[token];
            require(amount > _left, "Amount must be bigger than what's left");
            _safeTransferFrom(token, msg.sender, address(this), amount);
            _adminFee = amount * IContract(contractAddress).adminShare() / 10000;
            amount -= _adminFee;
            rewardRate[token] = (amount + _left) / DURATION;
        }
        treasury[token] += _adminFee;
        require(rewardRate[token] > 0, "Reward rate cannot be null");
    ...
    function setContractAddress(address _contractAddress) external {
        require(contractAddress == address(0x0) || factory == msg.sender, "Cannot set contract address");
        contractAddress = _contractAddress;
        factory = IContract(_contractAddress).businessBribeFactory();
    }

    function withdrawToDestination(address _token) external {
        address destination = IContract(contractAddress).veValuepool(_ve);
        require(destination != address(0x0), "Destination cannot be null");
        _safeTransfer(_token, destination, treasury[_token]);
        treasury[_token] = 0;
    }

    function _safeTransfer(address token, address to, uint256 value) internal {
        require(token.code.length > 0, "Bribe transfer error");
        (bool success, bytes memory data) =
        token.call(abi.encodeWithSelector(erc20.transfer.selector, to, value));
        require(success && (data.length == 0 || abi.decode(data, (bool))), "Bribe transfer failed");
    }
...
contract BusinessBribeFactory {
    address public last_gauge;
    address public contractAddress;

    function setContractAddress(address _contractAddress) external {
            require(contractAddress == address(0x0) || IAuth(contractAddress).devaddr_() == msg.sender, "Bribe Factory contract address failed");
        contractAddress = _contractAddress;
    }
    
    function setContractAddressAt(address _bribe) external {
        IMarketPlace(_bribe).setContractAddress(contractAddress);
    }
