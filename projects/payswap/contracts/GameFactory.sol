...
contract GameFactory {
    ...
    function getToken(uint _collectionId) external view returns(address) {
        return ticketInfo_[_collectionId].token;
    }
    //-------------------------------------------------------------------------
    ...
    function buyWithContract(
        ...
        (,,address _ticketGame,,uint _score,,,,,,,) = IGameNFT(gameMinter).gameInfo_(_tokenId);
...
contract GameMinter {
    ...
    function updateGameContract(
        ...
        if (_minutes == 0) gameInfo_[_tokenId].gameMinutes = 0;
    ...
    function updateScoreNDeadline(uint _tokenId, uint _score, uint _deadline) external {
        ...
        IGameNFT(gameFactory).updateTotalScore(
...
contract GameHelper is ERC721Pausable {
    ...
    function mintObject(string memory _objectName, uint _collectionId, uint _gameTokenId, uint[] memory _tokenIds) external {
        for (uint i = 0; i < _tokenIds.length; i++) {
    ...
    function _constructTokenURI(uint _tokenId, uint _collectionId, string[] memory _media, string[] memory optionNames, string[] memory optionValues) internal view returns(string memory) {
